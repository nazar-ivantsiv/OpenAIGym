...
class QTableMC:
    ...
    """ Implementation of tabular Q-learning algorithm with Monte-Carlo update. """
    ...
    def update_mc(self, trajectory):
        """ Incremental Monte-Carlo update for a single trajectory """
        g = 0.0
...
#                         print(f'episode: {e}/{episodes}, total_reward: {total_reward}')
                        break
                self.update_mc(trajectory)  # MC update
                ...
                if not (e % 1000) and e:
                    self.save_qtable(f'qtbl_mc_e{e}.npy')
            self.env.close()
...
#         self.env.seed(np.random.randint(1000))
    ...
    def save_qtable(self, q_table_fn='qtbl_mc.npy'):
        folder = 'checkpoints'
